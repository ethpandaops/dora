...
// It returns math.MaxInt64 if the value overflows the int64 range.
func GetBaseFeeAsUint64(baseFee any) uint64 {
	var result uint64

	switch fee := baseFee.(type) {
	case [32]byte:
		result = Uint256ToUint64(fee)
	default:
		// Try to use the Uint64 method if available
		if feeWithUint64, ok := fee.(interface{ Uint64() uint64 }); ok {
			result = feeWithUint64.Uint64()
		}
	}
