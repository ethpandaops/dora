...
func EpochOfSlot(slot uint64) uint64 {
	return slot / utils.Config.Chain.Config.SlotsPerEpoch
}

// DayOfSlot returns the corresponding day of a slot
func DayOfSlot(slot uint64) uint64 {
	return utils.Config.Chain.Config.SecondsPerSlot * slot / (24 * 3600)
}

// WeekOfSlot returns the corresponding week of a slot
func WeekOfSlot(slot uint64) uint64 {
	return utils.Config.Chain.Config.SecondsPerSlot * slot / (7 * 24 * 3600)
}

// SlotToTime returns a time.Time to slot
func SlotToTime(slot uint64) time.Time {
	return time.Unix(int64(utils.Config.Chain.GenesisTimestamp+slot*utils.Config.Chain.Config.SecondsPerSlot), 0)
}

// TimeToSlot returns time to slot in seconds
func TimeToSlot(timestamp uint64) uint64 {
	if utils.Config.Chain.GenesisTimestamp > timestamp {
		return 0
	}
	return (timestamp - utils.Config.Chain.GenesisTimestamp) / utils.Config.Chain.Config.SecondsPerSlot
}

func TimeToFirstSlotOfEpoch(timestamp uint64) uint64 {
	slot := TimeToSlot(timestamp)
	lastEpochOffset := slot % utils.Config.Chain.Config.SlotsPerEpoch
...
func EpochToTime(epoch uint64) time.Time {
	return time.Unix(int64(utils.Config.Chain.GenesisTimestamp+epoch*utils.Config.Chain.Config.SecondsPerSlot*utils.Config.Chain.Config.SlotsPerEpoch), 0)
}

// TimeToDay will return a days since genesis for an timestamp
func TimeToDay(timestamp uint64) uint64 {
	return uint64(time.Unix(int64(timestamp), 0).Sub(time.Unix(int64(utils.Config.Chain.GenesisTimestamp), 0)).Hours() / 24)
...
func DayToTime(day int64) time.Time {
	return time.Unix(int64(utils.Config.Chain.GenesisTimestamp), 0).Add(time.Hour * time.Duration(24*int(day)))
}

// TimeToEpoch will return an epoch for a given time
func TimeToEpoch(ts time.Time) int64 {
	if int64(utils.Config.Chain.GenesisTimestamp) > ts.Unix() {
		return 0
	}
	return (ts.Unix() - int64(utils.Config.Chain.GenesisTimestamp)) / int64(utils.Config.Chain.Config.SecondsPerSlot) / int64(utils.Config.Chain.Config.SlotsPerEpoch)
}
