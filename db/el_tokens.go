...
// GetElTokensFiltered returns tokens matching the given filter, paginated and
// ordered by id ASC. el_tokens is small (~hundreds of thousands of rows) so
// offset pagination with a total count is acceptable here. The first returned
// row carries the total count in its ID field.
