	...
	_, err := tx.ExecContext(ctx, EngineQuery(map[dbtypes.DBEngineType]string{
		dbtypes.DBEnginePgsql: `
			INSERT INTO orphaned_epochs (
				...
				blob_count, eth_gas_used, eth_gas_limit, voted_target_slashed
		...
		epoch.BlobCount, epoch.EthGasUsed, epoch.EthGasLimit, epoch.VotedTargetSlashed,
	)
	if err != nil {
		return err
	}
...
func StreamOrphanedEpochs(ctx context.Context, epoch uint64, cb func(duty *dbtypes.OrphanedEpoch)) error {
	...
	for rows.Next() {
		...
		err := rows.Scan(
			...
			&e.BlobCount, &e.EthGasUsed, &e.EthGasLimit, &e.VotedTargetSlashed,
		)
		if err != nil {
