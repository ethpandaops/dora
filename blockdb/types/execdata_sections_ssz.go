// Code generated by dynamic-ssz. DO NOT EDIT.
// Hash: 5a0ead28c6909e104289367705fbbd7bca366323caa0b95f186bc534918d6c17
// Version: v1.4.0-pre.2 (https://github.com/pk910/dynamic-ssz)
package types

import (
	"encoding/binary"
	"math"

	dynssz "github.com/pk910/dynamic-ssz"
	"github.com/pk910/dynamic-ssz/hasher"
	"github.com/pk910/dynamic-ssz/sszutils"
)

var _ = sszutils.ErrListTooBig

var _ = sszutils.Annotate[ReceiptMetaData](`ssz-static:"true"`)
var _ = sszutils.Annotate[BlockReceiptMeta](`ssz-static:"true"`)
var _ = sszutils.Annotate[StateChangeAccount](`ssz-static:"false"`)
var _ = sszutils.Annotate[FlatCallFrame](`ssz-static:"false"`)
var _ = sszutils.Annotate[EventData](`ssz-static:"false"`)

// MarshalSSZ marshals the *ReceiptMetaData to SSZ-encoded bytes.
func (t *ReceiptMetaData) MarshalSSZ() ([]byte, error) {
	return dynssz.GetGlobalDynSsz().MarshalSSZ(t)
}

// MarshalSSZTo marshals the *ReceiptMetaData to SSZ-encoded bytes, appending to the provided buffer.
func (t *ReceiptMetaData) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	dst = buf
	if t == nil {
		t = new(ReceiptMetaData)
	}
	{ // Static Field #0 'Version'
		dst = binary.LittleEndian.AppendUint16(dst, t.Version)
	}
	{ // Static Field #1 'Status'
		dst = append(dst, byte(t.Status))
	}
	{ // Static Field #2 'TxType'
		dst = append(dst, byte(t.TxType))
	}
	{ // Static Field #3 'CumulativeGasUsed'
		dst = binary.LittleEndian.AppendUint64(dst, t.CumulativeGasUsed)
	}
	{ // Static Field #4 'GasUsed'
		dst = binary.LittleEndian.AppendUint64(dst, t.GasUsed)
	}
	{ // Static Field #5 'EffectiveGasPrice'
		t := &t.EffectiveGasPrice
		dst = sszutils.MarshalUint64Slice(dst, t[:4])
	}
	{ // Static Field #6 'BlobGasUsed'
		dst = binary.LittleEndian.AppendUint64(dst, t.BlobGasUsed)
	}
	{ // Static Field #7 'LogsBloom'
		dst = append(dst, t.LogsBloom[:256]...)
	}
	{ // Static Field #8 'From'
		dst = append(dst, t.From[:20]...)
	}
	{ // Static Field #9 'To'
		dst = append(dst, t.To[:20]...)
	}
	{ // Static Field #10 'ContractAddress'
		dst = append(dst, t.ContractAddress[:20]...)
	}
	{ // Static Field #11 'HasContractAddr'
		dst = sszutils.MarshalBool(dst, t.HasContractAddr)
	}
	return dst, nil
}

// UnmarshalSSZ unmarshals the *ReceiptMetaData from SSZ-encoded bytes.
func (t *ReceiptMetaData) UnmarshalSSZ(buf []byte) (err error) {
	buflen := uint64(len(buf))
	if 377 != buflen {
		if 377 > buflen {
			return sszutils.ErrFixedFieldsEOFFn(buflen, 377)
		}
		return sszutils.ErrTrailingDataFn(buflen - 377)
	}
	{ // Field #0 'Version' (static)
		buf := buf[0:2]
		t.Version = binary.LittleEndian.Uint16(buf)
	}
	{ // Field #1 'Status' (static)
		buf := buf[2:3]
		t.Status = buf[0]
	}
	{ // Field #2 'TxType' (static)
		buf := buf[3:4]
		t.TxType = buf[0]
	}
	{ // Field #3 'CumulativeGasUsed' (static)
		buf := buf[4:12]
		t.CumulativeGasUsed = binary.LittleEndian.Uint64(buf)
	}
	{ // Field #4 'GasUsed' (static)
		buf := buf[12:20]
		t.GasUsed = binary.LittleEndian.Uint64(buf)
	}
	{ // Field #5 'EffectiveGasPrice' (static)
		buf := buf[20:52]
		val1 := t.EffectiveGasPrice
		sszutils.UnmarshalUint64Slice(val1[:4], buf)
		t.EffectiveGasPrice = val1
	}
	{ // Field #6 'BlobGasUsed' (static)
		buf := buf[52:60]
		t.BlobGasUsed = binary.LittleEndian.Uint64(buf)
	}
	{ // Field #7 'LogsBloom' (static)
		buf := buf[60:316]
		copy(t.LogsBloom[:], buf)
	}
	{ // Field #8 'From' (static)
		buf := buf[316:336]
		copy(t.From[:], buf)
	}
	{ // Field #9 'To' (static)
		buf := buf[336:356]
		copy(t.To[:], buf)
	}
	{ // Field #10 'ContractAddress' (static)
		buf := buf[356:376]
		copy(t.ContractAddress[:], buf)
	}
	{ // Field #11 'HasContractAddr' (static)
		buf := buf[376:377]
		if buf[0] != 1 && buf[0] != 0 {
			return sszutils.ErrInvalidBoolValueFn()
		}
		t.HasContractAddr = buf[0] == 1
	}
	return nil
}

// SizeSSZ returns the SSZ encoded size of the *ReceiptMetaData.
func (t *ReceiptMetaData) SizeSSZ() (size int) {
	return 377
}

// HashTreeRoot computes the SSZ hash tree root of the *ReceiptMetaData.
func (t *ReceiptMetaData) HashTreeRoot() (root [32]byte, err error) {
	err = hasher.WithDefaultHasher(func(hh sszutils.HashWalker) (err error) {
		err = t.HashTreeRootWith(hh)
		if err == nil {
			root, err = hh.HashRoot()
		}
		return
	})
	return
}

// HashTreeRootWith computes the SSZ hash tree root of the *ReceiptMetaData using the given hash walker.
func (t *ReceiptMetaData) HashTreeRootWith(hh sszutils.HashWalker) error {
	if t == nil {
		t = new(ReceiptMetaData)
	}
	idx := hh.StartTree(sszutils.TreeTypeNone)
	{ // Field #0 'Version'
		hh.PutUint16(t.Version)
	}
	{ // Field #1 'Status'
		hh.PutUint8(t.Status)
	}
	{ // Field #2 'TxType'
		hh.PutUint8(t.TxType)
	}
	{ // Field #3 'CumulativeGasUsed'
		hh.PutUint64(t.CumulativeGasUsed)
	}
	{ // Field #4 'GasUsed'
		hh.PutUint64(t.GasUsed)
	}
	{ // Field #5 'EffectiveGasPrice'
		t := &t.EffectiveGasPrice
		if root, err := t.HashTreeRoot(); err != nil {
			return sszutils.ErrorWithPath(err, "EffectiveGasPrice")
		} else {
			hh.AppendBytes32(root[:])
		}
	}
	{ // Field #6 'BlobGasUsed'
		hh.PutUint64(t.BlobGasUsed)
	}
	{ // Field #7 'LogsBloom'
		hh.PutBytes(t.LogsBloom[:256])
	}
	{ // Field #8 'From'
		hh.PutBytes(t.From[:20])
	}
	{ // Field #9 'To'
		hh.PutBytes(t.To[:20])
	}
	{ // Field #10 'ContractAddress'
		hh.PutBytes(t.ContractAddress[:20])
	}
	{ // Field #11 'HasContractAddr'
		hh.PutBool(t.HasContractAddr)
	}
	hh.Merkleize(idx)
	return nil
}

// MarshalSSZ marshals the *BlockReceiptMeta to SSZ-encoded bytes.
func (t *BlockReceiptMeta) MarshalSSZ() ([]byte, error) {
	return dynssz.GetGlobalDynSsz().MarshalSSZ(t)
}

// MarshalSSZTo marshals the *BlockReceiptMeta to SSZ-encoded bytes, appending to the provided buffer.
func (t *BlockReceiptMeta) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	dst = buf
	if t == nil {
		t = new(BlockReceiptMeta)
	}
	{ // Static Field #0 'Version'
		dst = binary.LittleEndian.AppendUint16(dst, t.Version)
	}
	{ // Static Field #1 'BlobGasPrice'
		dst = binary.LittleEndian.AppendUint64(dst, t.BlobGasPrice)
	}
	return dst, nil
}

// UnmarshalSSZ unmarshals the *BlockReceiptMeta from SSZ-encoded bytes.
func (t *BlockReceiptMeta) UnmarshalSSZ(buf []byte) (err error) {
	buflen := uint64(len(buf))
	if 10 != buflen {
		if 10 > buflen {
			return sszutils.ErrFixedFieldsEOFFn(buflen, 10)
		}
		return sszutils.ErrTrailingDataFn(buflen - 10)
	}
	{ // Field #0 'Version' (static)
		buf := buf[0:2]
		t.Version = binary.LittleEndian.Uint16(buf)
	}
	{ // Field #1 'BlobGasPrice' (static)
		buf := buf[2:10]
		t.BlobGasPrice = binary.LittleEndian.Uint64(buf)
	}
	return nil
}

// SizeSSZ returns the SSZ encoded size of the *BlockReceiptMeta.
func (t *BlockReceiptMeta) SizeSSZ() (size int) {
	return 10
}

// HashTreeRoot computes the SSZ hash tree root of the *BlockReceiptMeta.
func (t *BlockReceiptMeta) HashTreeRoot() (root [32]byte, err error) {
	err = hasher.WithDefaultHasher(func(hh sszutils.HashWalker) (err error) {
		err = t.HashTreeRootWith(hh)
		if err == nil {
			root, err = hh.HashRoot()
		}
		return
	})
	return
}

// HashTreeRootWith computes the SSZ hash tree root of the *BlockReceiptMeta using the given hash walker.
func (t *BlockReceiptMeta) HashTreeRootWith(hh sszutils.HashWalker) error {
	if t == nil {
		t = new(BlockReceiptMeta)
	}
	idx := hh.StartTree(sszutils.TreeTypeNone)
	{ // Field #0 'Version'
		hh.PutUint16(t.Version)
	}
	{ // Field #1 'BlobGasPrice'
		hh.PutUint64(t.BlobGasPrice)
	}
	hh.Merkleize(idx)
	return nil
}

// MarshalSSZ marshals the *StateChangeAccount to SSZ-encoded bytes.
func (t *StateChangeAccount) MarshalSSZ() ([]byte, error) {
	return dynssz.GetGlobalDynSsz().MarshalSSZ(t)
}

// MarshalSSZTo marshals the *StateChangeAccount to SSZ-encoded bytes, appending to the provided buffer.
func (t *StateChangeAccount) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	dst = buf
	zeroBytes := sszutils.ZeroBytes()
	if t == nil {
		t = new(StateChangeAccount)
	}
	dstlen := len(dst)
	{ // Static Field #0 'Address'
		dst = append(dst, t.Address[:20]...)
	}
	{ // Static Field #1 'Flags'
		dst = append(dst, byte(t.Flags))
	}
	{ // Static Field #2 'PreBalance'
		t := &t.PreBalance
		dst = sszutils.MarshalUint64Slice(dst, t[:4])
	}
	{ // Static Field #3 'PostBalance'
		t := &t.PostBalance
		dst = sszutils.MarshalUint64Slice(dst, t[:4])
	}
	{ // Static Field #4 'PreNonce'
		dst = binary.LittleEndian.AppendUint64(dst, t.PreNonce)
	}
	{ // Static Field #5 'PostNonce'
		dst = binary.LittleEndian.AppendUint64(dst, t.PostNonce)
	}
	// Offset Field #6 'PreCode'
	// Offset Field #7 'PostCode'
	// Offset Field #8 'Slots'
	dst = append(dst, zeroBytes[:12]...)
	{ // Dynamic Field #6 'PreCode'
		if off := uint64(len(dst)) - uint64(dstlen); off > math.MaxUint32 {
			return nil, sszutils.ErrOffsetOverflowFn(off)
		} else {
			binary.LittleEndian.PutUint32(dst[dstlen+101:], uint32(off))
		}
		vlen := len(t.PreCode)
		if vlen > 1048576 {
			return nil, sszutils.ErrorWithPath(sszutils.ErrListLengthFn(vlen, 1048576), "PreCode")
		}
		dst = append(dst, t.PreCode[:]...)
	}
	{ // Dynamic Field #7 'PostCode'
		if off := uint64(len(dst)) - uint64(dstlen); off > math.MaxUint32 {
			return nil, sszutils.ErrOffsetOverflowFn(off)
		} else {
			binary.LittleEndian.PutUint32(dst[dstlen+105:], uint32(off))
		}
		vlen := len(t.PostCode)
		if vlen > 1048576 {
			return nil, sszutils.ErrorWithPath(sszutils.ErrListLengthFn(vlen, 1048576), "PostCode")
		}
		dst = append(dst, t.PostCode[:]...)
	}
	{ // Dynamic Field #8 'Slots'
		if off := uint64(len(dst)) - uint64(dstlen); off > math.MaxUint32 {
			return nil, sszutils.ErrOffsetOverflowFn(off)
		} else {
			binary.LittleEndian.PutUint32(dst[dstlen+109:], uint32(off))
		}
		t := t.Slots
		vlen := len(t)
		if vlen > 1048576 {
			return nil, sszutils.ErrorWithPath(sszutils.ErrListLengthFn(vlen, 1048576), "Slots")
		}
		for idx1 := range vlen {
			t := &t[idx1]
			{ // Static Field #0 'Slot'
				dst = append(dst, t.Slot[:32]...)
			}
			{ // Static Field #1 'PreValue'
				dst = append(dst, t.PreValue[:32]...)
			}
			{ // Static Field #2 'PostValue'
				dst = append(dst, t.PostValue[:32]...)
			}
		}
	}
	return dst, nil
}

// UnmarshalSSZ unmarshals the *StateChangeAccount from SSZ-encoded bytes.
func (t *StateChangeAccount) UnmarshalSSZ(buf []byte) (err error) {
	buflen := uint64(len(buf))
	if buflen < 113 {
		return sszutils.ErrFixedFieldsEOFFn(buflen, 113)
	}
	maxOffset := uint32(sszutils.Min(buflen, math.MaxUint32))
	{ // Field #0 'Address' (static)
		buf := buf[0:20]
		copy(t.Address[:], buf)
	}
	{ // Field #1 'Flags' (static)
		buf := buf[20:21]
		t.Flags = buf[0]
	}
	{ // Field #2 'PreBalance' (static)
		buf := buf[21:53]
		val1 := t.PreBalance
		sszutils.UnmarshalUint64Slice(val1[:4], buf)
		t.PreBalance = val1
	}
	{ // Field #3 'PostBalance' (static)
		buf := buf[53:85]
		val2 := t.PostBalance
		sszutils.UnmarshalUint64Slice(val2[:4], buf)
		t.PostBalance = val2
	}
	{ // Field #4 'PreNonce' (static)
		buf := buf[85:93]
		t.PreNonce = binary.LittleEndian.Uint64(buf)
	}
	{ // Field #5 'PostNonce' (static)
		buf := buf[93:101]
		t.PostNonce = binary.LittleEndian.Uint64(buf)
	}
	// Field #6 'PreCode' (offset)
	offset6 := binary.LittleEndian.Uint32(buf[101:105])
	if offset6 != 113 {
		return sszutils.ErrorWithPath(sszutils.ErrFirstOffsetMismatchFn(offset6, 113), "PreCode:o")
	}
	// Field #7 'PostCode' (offset)
	offset7 := binary.LittleEndian.Uint32(buf[105:109])
	if offset7 < offset6 || offset7 > maxOffset {
		return sszutils.ErrorWithPath(sszutils.ErrOffsetOutOfRangeFn(offset7, offset6, buflen), "PostCode:o")
	}
	// Field #8 'Slots' (offset)
	offset8 := binary.LittleEndian.Uint32(buf[109:113])
	if offset8 < offset7 || offset8 > maxOffset {
		return sszutils.ErrorWithPath(sszutils.ErrOffsetOutOfRangeFn(offset8, offset7, buflen), "Slots:o")
	}
	{ // Field #6 'PreCode' (dynamic)
		buf := buf[offset6:offset7]
		if len(buf) > 1048576 {
			return sszutils.ErrorWithPath(sszutils.ErrListLengthFn(len(buf), 1048576), "PreCode")
		}
		t.PreCode = sszutils.ExpandSlice(t.PreCode, len(buf))
		copy(t.PreCode[:], buf)
	}
	{ // Field #7 'PostCode' (dynamic)
		buf := buf[offset7:offset8]
		if len(buf) > 1048576 {
			return sszutils.ErrorWithPath(sszutils.ErrListLengthFn(len(buf), 1048576), "PostCode")
		}
		t.PostCode = sszutils.ExpandSlice(t.PostCode, len(buf))
		copy(t.PostCode[:], buf)
	}
	{ // Field #8 'Slots' (dynamic)
		buf := buf[offset8:]
		val3 := t.Slots
		itemCount := len(buf) / 96
		if len(buf)%96 != 0 {
			return sszutils.ErrorWithPath(sszutils.ErrListNotAlignedFn(len(buf), 96), "Slots")
		}
		if itemCount > 1048576 {
			return sszutils.ErrorWithPath(sszutils.ErrListLengthFn(itemCount, 1048576), "Slots")
		}
		val3 = sszutils.ExpandSlice(val3, itemCount)
		for idx1 := range itemCount {
			val4 := val3[idx1]
			buf := buf[96*idx1 : 96*(idx1+1)]
			buflen := uint64(len(buf))
			if 96 != buflen {
				if 96 > buflen {
					return sszutils.ErrorWithPathf(sszutils.ErrFixedFieldsEOFFn(buflen, 96), "Slots[%d]", idx1)
				}
				return sszutils.ErrorWithPathf(sszutils.ErrTrailingDataFn(buflen-96), "Slots[%d]", idx1)
			}
			{ // Field #0 'Slot' (static)
				buf := buf[0:32]
				copy(val4.Slot[:], buf)
			}
			{ // Field #1 'PreValue' (static)
				buf := buf[32:64]
				copy(val4.PreValue[:], buf)
			}
			{ // Field #2 'PostValue' (static)
				buf := buf[64:96]
				copy(val4.PostValue[:], buf)
			}
			val3[idx1] = val4
		}
		t.Slots = val3
	}
	return nil
}

// SizeSSZ returns the SSZ encoded size of the *StateChangeAccount.
func (t *StateChangeAccount) SizeSSZ() (size int) {
	if t == nil {
		t = new(StateChangeAccount)
	}
	// Field #0 'Address' static (20 bytes)
	// Field #1 'Flags' static (1 bytes)
	// Field #2 'PreBalance' static (32 bytes)
	// Field #3 'PostBalance' static (32 bytes)
	// Field #4 'PreNonce' static (8 bytes)
	// Field #5 'PostNonce' static (8 bytes)
	// Field #6 'PreCode' offset (4 bytes)
	// Field #7 'PostCode' offset (4 bytes)
	// Field #8 'Slots' offset (4 bytes)
	size += 113
	{ // Dynamic field #6 'PreCode'
		size += len(t.PreCode)
	}
	{ // Dynamic field #7 'PostCode'
		size += len(t.PostCode)
	}
	{ // Dynamic field #8 'Slots'
		size += len(t.Slots) * 96
	}
	return size
}

// HashTreeRoot computes the SSZ hash tree root of the *StateChangeAccount.
func (t *StateChangeAccount) HashTreeRoot() (root [32]byte, err error) {
	err = hasher.WithDefaultHasher(func(hh sszutils.HashWalker) (err error) {
		err = t.HashTreeRootWith(hh)
		if err == nil {
			root, err = hh.HashRoot()
		}
		return
	})
	return
}

// HashTreeRootWith computes the SSZ hash tree root of the *StateChangeAccount using the given hash walker.
func (t *StateChangeAccount) HashTreeRootWith(hh sszutils.HashWalker) error {
	if t == nil {
		t = new(StateChangeAccount)
	}
	idx := hh.StartTree(sszutils.TreeTypeNone)
	{ // Field #0 'Address'
		hh.PutBytes(t.Address[:20])
	}
	{ // Field #1 'Flags'
		hh.PutUint8(t.Flags)
	}
	{ // Field #2 'PreBalance'
		t := &t.PreBalance
		if root, err := t.HashTreeRoot(); err != nil {
			return sszutils.ErrorWithPath(err, "PreBalance")
		} else {
			hh.AppendBytes32(root[:])
		}
	}
	{ // Field #3 'PostBalance'
		t := &t.PostBalance
		if root, err := t.HashTreeRoot(); err != nil {
			return sszutils.ErrorWithPath(err, "PostBalance")
		} else {
			hh.AppendBytes32(root[:])
		}
	}
	{ // Field #4 'PreNonce'
		hh.PutUint64(t.PreNonce)
	}
	{ // Field #5 'PostNonce'
		hh.PutUint64(t.PostNonce)
	}
	{ // Field #6 'PreCode'
		vlen := uint64(len(t.PreCode))
		if vlen > 1048576 {
			return sszutils.ErrorWithPath(sszutils.ErrListLengthFn(vlen, 1048576), "PreCode")
		}
		idx := hh.StartTree(sszutils.TreeTypeBinary)
		hh.AppendBytes32(t.PreCode[:])
		hh.MerkleizeWithMixin(idx, vlen, sszutils.CalculateLimit(1048576, vlen, 1))
	}
	{ // Field #7 'PostCode'
		vlen := uint64(len(t.PostCode))
		if vlen > 1048576 {
			return sszutils.ErrorWithPath(sszutils.ErrListLengthFn(vlen, 1048576), "PostCode")
		}
		idx := hh.StartTree(sszutils.TreeTypeBinary)
		hh.AppendBytes32(t.PostCode[:])
		hh.MerkleizeWithMixin(idx, vlen, sszutils.CalculateLimit(1048576, vlen, 1))
	}
	{ // Field #8 'Slots'
		t := t.Slots
		vlen := uint64(len(t))
		if vlen > 1048576 {
			return sszutils.ErrorWithPath(sszutils.ErrListLengthFn(vlen, 1048576), "Slots")
		}
		idx := hh.StartTree(sszutils.TreeTypeBinary)
		for idx1 := range len(t) {
			t := &t[idx1]
			idx := hh.StartTree(sszutils.TreeTypeNone)
			{ // Field #0 'Slot'
				hh.PutBytes(t.Slot[:32])
			}
			{ // Field #1 'PreValue'
				hh.PutBytes(t.PreValue[:32])
			}
			{ // Field #2 'PostValue'
				hh.PutBytes(t.PostValue[:32])
			}
			hh.Merkleize(idx)
			if (idx1+1)%256 == 0 {
				hh.Collapse()
			}
		}
		hh.MerkleizeWithMixin(idx, vlen, sszutils.CalculateLimit(1048576, vlen, 32))
	}
	hh.Merkleize(idx)
	return nil
}

// MarshalSSZ marshals the *FlatCallFrame to SSZ-encoded bytes.
func (t *FlatCallFrame) MarshalSSZ() ([]byte, error) {
	return dynssz.GetGlobalDynSsz().MarshalSSZ(t)
}

// MarshalSSZTo marshals the *FlatCallFrame to SSZ-encoded bytes, appending to the provided buffer.
func (t *FlatCallFrame) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	dst = buf
	zeroBytes := sszutils.ZeroBytes()
	if t == nil {
		t = new(FlatCallFrame)
	}
	dstlen := len(dst)
	{ // Static Field #0 'Depth'
		dst = binary.LittleEndian.AppendUint16(dst, t.Depth)
	}
	{ // Static Field #1 'Type'
		dst = append(dst, byte(t.Type))
	}
	{ // Static Field #2 'From'
		dst = append(dst, t.From[:20]...)
	}
	{ // Static Field #3 'To'
		dst = append(dst, t.To[:20]...)
	}
	{ // Static Field #4 'Value'
		t := &t.Value
		dst = sszutils.MarshalUint64Slice(dst, t[:4])
	}
	{ // Static Field #5 'Gas'
		dst = binary.LittleEndian.AppendUint64(dst, t.Gas)
	}
	{ // Static Field #6 'GasUsed'
		dst = binary.LittleEndian.AppendUint64(dst, t.GasUsed)
	}
	{ // Static Field #7 'Status'
		dst = append(dst, byte(t.Status))
	}
	// Offset Field #8 'Input'
	// Offset Field #9 'Output'
	// Offset Field #10 'Error'
	dst = append(dst, zeroBytes[:12]...)
	{ // Dynamic Field #8 'Input'
		if off := uint64(len(dst)) - uint64(dstlen); off > math.MaxUint32 {
			return nil, sszutils.ErrOffsetOverflowFn(off)
		} else {
			binary.LittleEndian.PutUint32(dst[dstlen+92:], uint32(off))
		}
		vlen := len(t.Input)
		if vlen > 1048576 {
			return nil, sszutils.ErrorWithPath(sszutils.ErrListLengthFn(vlen, 1048576), "Input")
		}
		dst = append(dst, t.Input[:]...)
	}
	{ // Dynamic Field #9 'Output'
		if off := uint64(len(dst)) - uint64(dstlen); off > math.MaxUint32 {
			return nil, sszutils.ErrOffsetOverflowFn(off)
		} else {
			binary.LittleEndian.PutUint32(dst[dstlen+96:], uint32(off))
		}
		vlen := len(t.Output)
		if vlen > 1048576 {
			return nil, sszutils.ErrorWithPath(sszutils.ErrListLengthFn(vlen, 1048576), "Output")
		}
		dst = append(dst, t.Output[:]...)
	}
	{ // Dynamic Field #10 'Error'
		if off := uint64(len(dst)) - uint64(dstlen); off > math.MaxUint32 {
			return nil, sszutils.ErrOffsetOverflowFn(off)
		} else {
			binary.LittleEndian.PutUint32(dst[dstlen+100:], uint32(off))
		}
		vlen := len(t.Error)
		if vlen > 1048576 {
			return nil, sszutils.ErrorWithPath(sszutils.ErrListLengthFn(vlen, 1048576), "Error")
		}
		dst = append(dst, t.Error[:]...)
	}
	return dst, nil
}

// UnmarshalSSZ unmarshals the *FlatCallFrame from SSZ-encoded bytes.
func (t *FlatCallFrame) UnmarshalSSZ(buf []byte) (err error) {
	buflen := uint64(len(buf))
	if buflen < 104 {
		return sszutils.ErrFixedFieldsEOFFn(buflen, 104)
	}
	maxOffset := uint32(sszutils.Min(buflen, math.MaxUint32))
	{ // Field #0 'Depth' (static)
		buf := buf[0:2]
		t.Depth = binary.LittleEndian.Uint16(buf)
	}
	{ // Field #1 'Type' (static)
		buf := buf[2:3]
		t.Type = buf[0]
	}
	{ // Field #2 'From' (static)
		buf := buf[3:23]
		copy(t.From[:], buf)
	}
	{ // Field #3 'To' (static)
		buf := buf[23:43]
		copy(t.To[:], buf)
	}
	{ // Field #4 'Value' (static)
		buf := buf[43:75]
		val1 := t.Value
		sszutils.UnmarshalUint64Slice(val1[:4], buf)
		t.Value = val1
	}
	{ // Field #5 'Gas' (static)
		buf := buf[75:83]
		t.Gas = binary.LittleEndian.Uint64(buf)
	}
	{ // Field #6 'GasUsed' (static)
		buf := buf[83:91]
		t.GasUsed = binary.LittleEndian.Uint64(buf)
	}
	{ // Field #7 'Status' (static)
		buf := buf[91:92]
		t.Status = buf[0]
	}
	// Field #8 'Input' (offset)
	offset8 := binary.LittleEndian.Uint32(buf[92:96])
	if offset8 != 104 {
		return sszutils.ErrorWithPath(sszutils.ErrFirstOffsetMismatchFn(offset8, 104), "Input:o")
	}
	// Field #9 'Output' (offset)
	offset9 := binary.LittleEndian.Uint32(buf[96:100])
	if offset9 < offset8 || offset9 > maxOffset {
		return sszutils.ErrorWithPath(sszutils.ErrOffsetOutOfRangeFn(offset9, offset8, buflen), "Output:o")
	}
	// Field #10 'Error' (offset)
	offset10 := binary.LittleEndian.Uint32(buf[100:104])
	if offset10 < offset9 || offset10 > maxOffset {
		return sszutils.ErrorWithPath(sszutils.ErrOffsetOutOfRangeFn(offset10, offset9, buflen), "Error:o")
	}
	{ // Field #8 'Input' (dynamic)
		buf := buf[offset8:offset9]
		if len(buf) > 1048576 {
			return sszutils.ErrorWithPath(sszutils.ErrListLengthFn(len(buf), 1048576), "Input")
		}
		t.Input = sszutils.ExpandSlice(t.Input, len(buf))
		copy(t.Input[:], buf)
	}
	{ // Field #9 'Output' (dynamic)
		buf := buf[offset9:offset10]
		if len(buf) > 1048576 {
			return sszutils.ErrorWithPath(sszutils.ErrListLengthFn(len(buf), 1048576), "Output")
		}
		t.Output = sszutils.ExpandSlice(t.Output, len(buf))
		copy(t.Output[:], buf)
	}
	{ // Field #10 'Error' (dynamic)
		buf := buf[offset10:]
		if len(buf) > 1048576 {
			return sszutils.ErrorWithPath(sszutils.ErrListLengthFn(len(buf), 1048576), "Error")
		}
		t.Error = string(buf)
	}
	return nil
}

// SizeSSZ returns the SSZ encoded size of the *FlatCallFrame.
func (t *FlatCallFrame) SizeSSZ() (size int) {
	if t == nil {
		t = new(FlatCallFrame)
	}
	// Field #0 'Depth' static (2 bytes)
	// Field #1 'Type' static (1 bytes)
	// Field #2 'From' static (20 bytes)
	// Field #3 'To' static (20 bytes)
	// Field #4 'Value' static (32 bytes)
	// Field #5 'Gas' static (8 bytes)
	// Field #6 'GasUsed' static (8 bytes)
	// Field #7 'Status' static (1 bytes)
	// Field #8 'Input' offset (4 bytes)
	// Field #9 'Output' offset (4 bytes)
	// Field #10 'Error' offset (4 bytes)
	size += 104
	{ // Dynamic field #8 'Input'
		size += len(t.Input)
	}
	{ // Dynamic field #9 'Output'
		size += len(t.Output)
	}
	{ // Dynamic field #10 'Error'
		size += len(t.Error)
	}
	return size
}

// HashTreeRoot computes the SSZ hash tree root of the *FlatCallFrame.
func (t *FlatCallFrame) HashTreeRoot() (root [32]byte, err error) {
	err = hasher.WithDefaultHasher(func(hh sszutils.HashWalker) (err error) {
		err = t.HashTreeRootWith(hh)
		if err == nil {
			root, err = hh.HashRoot()
		}
		return
	})
	return
}

// HashTreeRootWith computes the SSZ hash tree root of the *FlatCallFrame using the given hash walker.
func (t *FlatCallFrame) HashTreeRootWith(hh sszutils.HashWalker) error {
	if t == nil {
		t = new(FlatCallFrame)
	}
	idx := hh.StartTree(sszutils.TreeTypeNone)
	{ // Field #0 'Depth'
		hh.PutUint16(t.Depth)
	}
	{ // Field #1 'Type'
		hh.PutUint8(t.Type)
	}
	{ // Field #2 'From'
		hh.PutBytes(t.From[:20])
	}
	{ // Field #3 'To'
		hh.PutBytes(t.To[:20])
	}
	{ // Field #4 'Value'
		t := &t.Value
		if root, err := t.HashTreeRoot(); err != nil {
			return sszutils.ErrorWithPath(err, "Value")
		} else {
			hh.AppendBytes32(root[:])
		}
	}
	{ // Field #5 'Gas'
		hh.PutUint64(t.Gas)
	}
	{ // Field #6 'GasUsed'
		hh.PutUint64(t.GasUsed)
	}
	{ // Field #7 'Status'
		hh.PutUint8(t.Status)
	}
	{ // Field #8 'Input'
		vlen := uint64(len(t.Input))
		if vlen > 1048576 {
			return sszutils.ErrorWithPath(sszutils.ErrListLengthFn(vlen, 1048576), "Input")
		}
		idx := hh.StartTree(sszutils.TreeTypeBinary)
		hh.AppendBytes32(t.Input[:])
		hh.MerkleizeWithMixin(idx, vlen, sszutils.CalculateLimit(1048576, vlen, 1))
	}
	{ // Field #9 'Output'
		vlen := uint64(len(t.Output))
		if vlen > 1048576 {
			return sszutils.ErrorWithPath(sszutils.ErrListLengthFn(vlen, 1048576), "Output")
		}
		idx := hh.StartTree(sszutils.TreeTypeBinary)
		hh.AppendBytes32(t.Output[:])
		hh.MerkleizeWithMixin(idx, vlen, sszutils.CalculateLimit(1048576, vlen, 1))
	}
	{ // Field #10 'Error'
		vlen := uint64(len(t.Error))
		if vlen > 1048576 {
			return sszutils.ErrorWithPath(sszutils.ErrListLengthFn(vlen, 1048576), "Error")
		}
		idx := hh.StartTree(sszutils.TreeTypeBinary)
		hh.AppendBytes32([]byte(t.Error))
		hh.MerkleizeWithMixin(idx, vlen, sszutils.CalculateLimit(1048576, vlen, 1))
	}
	hh.Merkleize(idx)
	return nil
}

// MarshalSSZ marshals the *EventData to SSZ-encoded bytes.
func (t *EventData) MarshalSSZ() ([]byte, error) {
	return dynssz.GetGlobalDynSsz().MarshalSSZ(t)
}

// MarshalSSZTo marshals the *EventData to SSZ-encoded bytes, appending to the provided buffer.
func (t *EventData) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	dst = buf
	if t == nil {
		t = new(EventData)
	}
	dstlen := len(dst)
	{ // Static Field #0 'EventIndex'
		dst = binary.LittleEndian.AppendUint32(dst, t.EventIndex)
	}
	{ // Static Field #1 'Source'
		dst = append(dst, t.Source[:20]...)
	}
	// Offset Field #2 'Topics'
	// Offset Field #3 'Data'
	dst = append(dst, 0, 0, 0, 0, 0, 0, 0, 0)
	{ // Dynamic Field #2 'Topics'
		if off := uint64(len(dst)) - uint64(dstlen); off > math.MaxUint32 {
			return nil, sszutils.ErrOffsetOverflowFn(off)
		} else {
			binary.LittleEndian.PutUint32(dst[dstlen+24:], uint32(off))
		}
		t := t.Topics
		vlen := len(t)
		if vlen > 5 {
			return nil, sszutils.ErrorWithPath(sszutils.ErrListLengthFn(vlen, 5), "Topics")
		}
		for idx1 := range vlen {
			vlen := len(t[idx1])
			if vlen > 32 {
				return nil, sszutils.ErrorWithPathf(sszutils.ErrVectorLengthFn(vlen, 32), "Topics[%d]", idx1)
			}
			dst = append(dst, t[idx1][:vlen]...)
			if vlen < 32 {
				dst = sszutils.AppendZeroPadding(dst, (32-vlen)*1)
			}
		}
	}
	{ // Dynamic Field #3 'Data'
		if off := uint64(len(dst)) - uint64(dstlen); off > math.MaxUint32 {
			return nil, sszutils.ErrOffsetOverflowFn(off)
		} else {
			binary.LittleEndian.PutUint32(dst[dstlen+28:], uint32(off))
		}
		vlen := len(t.Data)
		if vlen > 1048576 {
			return nil, sszutils.ErrorWithPath(sszutils.ErrListLengthFn(vlen, 1048576), "Data")
		}
		dst = append(dst, t.Data[:]...)
	}
	return dst, nil
}

// UnmarshalSSZ unmarshals the *EventData from SSZ-encoded bytes.
func (t *EventData) UnmarshalSSZ(buf []byte) (err error) {
	buflen := uint64(len(buf))
	if buflen < 32 {
		return sszutils.ErrFixedFieldsEOFFn(buflen, 32)
	}
	maxOffset := uint32(sszutils.Min(buflen, math.MaxUint32))
	{ // Field #0 'EventIndex' (static)
		buf := buf[0:4]
		t.EventIndex = binary.LittleEndian.Uint32(buf)
	}
	{ // Field #1 'Source' (static)
		buf := buf[4:24]
		copy(t.Source[:], buf)
	}
	// Field #2 'Topics' (offset)
	offset2 := binary.LittleEndian.Uint32(buf[24:28])
	if offset2 != 32 {
		return sszutils.ErrorWithPath(sszutils.ErrFirstOffsetMismatchFn(offset2, 32), "Topics:o")
	}
	// Field #3 'Data' (offset)
	offset3 := binary.LittleEndian.Uint32(buf[28:32])
	if offset3 < offset2 || offset3 > maxOffset {
		return sszutils.ErrorWithPath(sszutils.ErrOffsetOutOfRangeFn(offset3, offset2, buflen), "Data:o")
	}
	{ // Field #2 'Topics' (dynamic)
		buf := buf[offset2:offset3]
		val1 := t.Topics
		itemCount := len(buf) / 32
		if len(buf)%32 != 0 {
			return sszutils.ErrorWithPath(sszutils.ErrListNotAlignedFn(len(buf), 32), "Topics")
		}
		if itemCount > 5 {
			return sszutils.ErrorWithPath(sszutils.ErrListLengthFn(itemCount, 5), "Topics")
		}
		val1 = sszutils.ExpandSlice(val1, itemCount)
		for idx1 := range itemCount {
			buf := buf[32*idx1 : 32*(idx1+1)]
			val1[idx1] = sszutils.ExpandSlice(val1[idx1], 32)
			copy(val1[idx1][:], buf)
		}
		t.Topics = val1
	}
	{ // Field #3 'Data' (dynamic)
		buf := buf[offset3:]
		if len(buf) > 1048576 {
			return sszutils.ErrorWithPath(sszutils.ErrListLengthFn(len(buf), 1048576), "Data")
		}
		t.Data = sszutils.ExpandSlice(t.Data, len(buf))
		copy(t.Data[:], buf)
	}
	return nil
}

// SizeSSZ returns the SSZ encoded size of the *EventData.
func (t *EventData) SizeSSZ() (size int) {
	if t == nil {
		t = new(EventData)
	}
	// Field #0 'EventIndex' static (4 bytes)
	// Field #1 'Source' static (20 bytes)
	// Field #2 'Topics' offset (4 bytes)
	// Field #3 'Data' offset (4 bytes)
	size += 32
	{ // Dynamic field #2 'Topics'
		size += len(t.Topics) * 32
	}
	{ // Dynamic field #3 'Data'
		size += len(t.Data)
	}
	return size
}

// HashTreeRoot computes the SSZ hash tree root of the *EventData.
func (t *EventData) HashTreeRoot() (root [32]byte, err error) {
	err = hasher.WithDefaultHasher(func(hh sszutils.HashWalker) (err error) {
		err = t.HashTreeRootWith(hh)
		if err == nil {
			root, err = hh.HashRoot()
		}
		return
	})
	return
}

// HashTreeRootWith computes the SSZ hash tree root of the *EventData using the given hash walker.
func (t *EventData) HashTreeRootWith(hh sszutils.HashWalker) error {
	if t == nil {
		t = new(EventData)
	}
	idx := hh.StartTree(sszutils.TreeTypeNone)
	{ // Field #0 'EventIndex'
		hh.PutUint32(t.EventIndex)
	}
	{ // Field #1 'Source'
		hh.PutBytes(t.Source[:20])
	}
	{ // Field #2 'Topics'
		t := t.Topics
		vlen := uint64(len(t))
		if vlen > 5 {
			return sszutils.ErrorWithPath(sszutils.ErrListLengthFn(vlen, 5), "Topics")
		}
		idx := hh.StartTree(sszutils.TreeTypeBinary)
		for idx1 := range len(t) {
			vlen := len(t[idx1])
			if vlen > 32 {
				return sszutils.ErrorWithPathf(sszutils.ErrVectorLengthFn(vlen, 32), "Topics[%d]", idx1)
			}
			val := t[idx1][:vlen:vlen]
			if vlen < 32 {
				val = sszutils.AppendZeroPadding(val, (32-vlen)*1)
			}
			hh.PutBytes(val[:32])
			if (idx1+1)%256 == 0 {
				hh.Collapse()
			}
		}
		hh.MerkleizeWithMixin(idx, vlen, sszutils.CalculateLimit(5, vlen, 32))
	}
	{ // Field #3 'Data'
		vlen := uint64(len(t.Data))
		if vlen > 1048576 {
			return sszutils.ErrorWithPath(sszutils.ErrListLengthFn(vlen, 1048576), "Data")
		}
		idx := hh.StartTree(sszutils.TreeTypeBinary)
		hh.AppendBytes32(t.Data[:])
		hh.MerkleizeWithMixin(idx, vlen, sszutils.CalculateLimit(1048576, vlen, 1))
	}
	hh.Merkleize(idx)
	return nil
}
